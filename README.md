...
| 参数 | 值 |
|------|-----|
| 时钟源 | APB1 定时器时钟 (72 MHz)，预分频 71 → 1 MHz (1 µs/tick) |
| 自动重装载 | 65535 (最大 65.535 ms) |
| 模式 | PWM 输入模式，CH1 上升沿 → CH2 下降沿 |
| 测距公式 | `distance(cm) = pulse_width(µs) / 58` |
| 有效范围 | 2 cm ~ 400 cm |

> ⚠️ **时钟说明**：APB1 外设时钟 = HCLK/2 = 36 MHz，但 APB1 预分频系数 ≠ 1 时，定时器时钟自动 ×2，因此 TIM2 实际输入时钟 = **72 MHz**。

### TIM3 — LED PWM 调光

| 参数 | 值 |
|------|-----|
| 时钟源 | APB1 定时器时钟 (72 MHz)，预分频 719 → 100 kHz |
| 自动重装载 | 99 → PWM 频率 = 1 kHz |
| 模式 | PWM Mode 1，高电平有效 |
| 占空比范围 | 0% ~ 99%（CCR 0 ~ 99，共 100 级） |

> ⚠️ **时钟说明**：同 TIM2，TIM3 实际输入时钟 = **72 MHz**（而非 APB1 外设时钟的 36 MHz）。
